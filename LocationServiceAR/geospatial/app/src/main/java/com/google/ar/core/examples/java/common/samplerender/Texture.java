...
public class Texture implements Closeable {
  ...
  public static Texture createFromBitmap(SampleRender render, Bitmap bitmap, WrapMode wrapMode, ColorFormat colorFormat) {
    Texture texture = new Texture(render, Target.TEXTURE_2D, wrapMode);
    try {
      // Convert the provided bitmap to ARGB_8888 configuration if necessary
      bitmap = convertBitmapToConfig(bitmap, Bitmap.Config.ARGB_8888);
      ByteBuffer buffer = ByteBuffer.allocateDirect(bitmap.getByteCount());
      bitmap.copyPixelsToBuffer(buffer);
      buffer.rewind();
      ...
      // Recycle the bitmap only if it was converted and created internally
      bitmap.recycle();
    }
    return texture;
  }
